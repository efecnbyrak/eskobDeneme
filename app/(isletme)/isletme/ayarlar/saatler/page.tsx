...
export default function CalismaSaatleriSayfasi() {
  ...
  const [saatler, setSaatler] = useState<CalismaSaatleri>(VARSAYILAN)
  const [yukleniyor, setYukleniyor] = useState(false)
  const [yukleniyorVeri, setYukleniyorVeri] = useState(true)
  const [esnafId, setEsnafId] = useState<number | null>(null)

  useEffect(() => {
    fetch('/api/esnaf')
      .then((r) => r.json())
      .then((json) => {
        const esnaf = json?.data ?? json
        if (esnaf?.id) {
          setEsnafId(esnaf.id)
          if (esnaf.calisma && typeof esnaf.calisma === 'object') {
            setSaatler({ ...VARSAYILAN, ...esnaf.calisma })
          }
        }
      })
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 animate-pulse">
          {[...Array(7)].map((_, i) => (
            <div key={i} className="h-14 bg-slate-100 rounded-lg mb-3" />
          ))}
        </div>
      </div>
    )
          ...
          return (
            <div key={key} className="flex items-center gap-4 px-6 py-4">
              ...
              <button
                ...
                className={`relative w-10 h-5 rounded-full transition-colors shrink-0 ${gun.acik ? 'bg-indigo-500' : 'bg-slate-200'}`}
              >
                <span

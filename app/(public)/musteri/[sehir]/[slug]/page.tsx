...
export default async function MusteriEsnafProfilSayfasi({ params }: Props) {
  const { slug } = await params
  ...
  if (!oturum?.user) {
    redirect(`/musteri/giris?callbackUrl=/musteri/${esnaf.sehir.toLowerCase()}/${esnaf.slug}`)
  }
  ...
  const sayfaUrl = `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/musteri/${esnaf.sehir.toLowerCase()}/${esnaf.slug}`

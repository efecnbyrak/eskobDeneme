...
const MENU_ITEMS = [
  ...
  {
    href: '/isletme/panel/kampanyalar',
    label: 'Kampanyalar',
    ikon: (
      <svg className="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <aside className="w-[240px] min-h-screen bg-white border-r border-[var(--color-border)] flex flex-col shrink-0">
      ...
      <div className="p-5 border-b border-[var(--color-border)]">
        <Link href="/" className="flex items-center gap-2.5">
          <span className="w-8 h-8 rounded-lg bg-[var(--color-primary)] text-white flex items-center justify-center text-sm font-bold font-display shrink-0">EV</span>
          ...
          {MENU_ITEMS.map((item) => {
            ...
            return (
              <li key={item.href}>
                ...
                >
                  <span className={cn('shrink-0', aktif ? 'text-[var(--color-primary)]' : '')}>
                    {item.ikon}
                  </span>
                  {item.label}
                </Link>
